...
% randperm
evenNum = 10:2:20; 
rand_evens = evenNum(randperm(length(evenNum)));
% length(evenNum) outputs the length of the original array  (i.e. 6)
% randperm(length(evenNum)) outputs a random order of the indices in the
% array (e.g. [2 6 3 1 4 5]
% evenNum(randperm(length(evenNum))) reorders the array to match the random
% indices generated by randperm. This is the equivalent of saying
% evenNum([2 6 3 1 4 5]) where the original evenNum is reordered to have
% the value in the second index swap to the first index and etc

%% 11
% strfind displays the indices at which the character 'm' appears in the
% string 
